...
from scheduler_service.util.sched_exceptions import FieldTimeAvailabilityError, CodeLogicError
from scheduler_service.util.schedule_util import roundrobin, getConnectedDivisionGroup, find_ge, find_le, all_same
...
class TournamentFieldTimeScheduleGenerator:
    ...
    def generateSchedule(self, totalmatch_list):
        ...
        for connected_div_list in self.connected_div_components:
            ...
            flatmatch_list = [
                {'ROUND_ID': z['round_id'], 'HOME': p['HOME'], 'AWAY': p['AWAY'], 'DIV_ID': x[self.idproperty]} for x in
                connecteddiv_match_list for y in x['match_list'] for z in y for p in z['game_team']]
            ...
            #      for j in items1:
            #         print key, key1, j
            grouped_match_list = [{'round_id': rkey, 'match_list': [
                [{'home': x['HOME'], 'away': x['AWAY'], self.idproperty: dkey} for x in ditems] for dkey, ditems in
                groupby(ritems, key=itemgetter('DIV_ID'))]} for rkey, ritems in
                                  groupby(sorted_flatmatch_list, key=itemgetter('ROUND_ID'))]
            ...
            fieldset = reduce(set.union,
                              map(set, [self.divinfo_list[self.dindexerGet(x)]['divfield_list'] for x in
                                        connected_div_list]))
            ...
            for round_games in grouped_match_list:
                ...
                for rrgame in rrgenobj:
                    ...
                    nextmin_datetime = self.getcandidate_daytime(div_id, home, away, latest_endtime - gameinterval,
                                                                 mingap_time)
                    ...
                    logging.debug("tournftscheduler:generate:assignment success rrgame %s gameday %d field %d slot %d",
                                  rrgame, efieldday_id, efield, eslot)
    ...
    def generateElimSchedule(self, totalmatch_list):
        ...
        for connected_div_list in self.connected_div_components:
            ...
            connecteddiv_matchrange_list = [{'div_id': totalmatch_list[tmindexerGet(x)]['div_id'],
                                             'match_id_range': totalmatch_list[tmindexerGet(x)]['match_id_range']} for x
                                            in connected_div_list]
            ...
            grouped_match_list = [{'absround_id': arkey,
                                   'match_list': [[{'home': y['home'], 'away': y['away'], 'div_id': y['div_id'],
                                                    'match_id': y['match_id'],
                                                    'comment': y['comment'], 'round': y['round']} for y in
                                                   x['match_list']] for x in aritems]}
            ...
            fieldset = reduce(set.union,
                              map(set, [self.divinfo_list[self.dindexerGet(x)]['divfield_list'] for x in
                                        connected_div_list]))
            field_list = list(fieldset)
            endtime_list = [(f, parser.parse(self.fieldinfo_list[self.findexerGet(f)]['end_time'].strftime('%Y-%m-%d %H:%M:%S'))) for f in field_list]
            latest_endtime = max(endtime_list, key=itemgetter(1))[1]
            ...
            for round_games in grouped_match_list:
                ...
                for rrgame in rrgenobj:
                    ...
                    # get absolute datetime that satisfies gaptime requirement
                    nextmin_datetime = self.getElimcandidate_daytime(div_id, home, away, field_list,
                                                                     latest_endtime - gameinterval, mingap_time,
                    ...
                    datesortedfield_list = self.datesort_fields(mindate_tuple,
                                                                field_list)
                    logging.debug(
                        "next mindatetime=%s datesortedfield_list=%s" % (nextmin_datetime, datesortedfield_list))
                    # current_fieldday_id = search_tuple[0]
                    # current_start = search_tuple[1]
                    # start time calc needs to be done here as start times for fields may change based on gameday
                    # if check in the list comprehension below exists as slotstatus_list[index] might be None if there is a closed_list
        ...
        for f in self.fieldinfo_list:
            ...
            if self.tourn_type == 'RR' and totalfielddays < totalgamedays:
                raise FieldTimeAvailabilityError(
                    "Note enough total fielddays %d to cover required totalgamedays" % (totalfielddays,),
                    totalgamedays_list)

            default_start_time = parser.parse(f['start_time'].strftime('%Y-%m-%d %H:%M:%S'))
            default_end_time = parser.parse(f['end_time'].strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def initTeamTimeGap_list(self, div_list):
        ...
        self.timegap_indexerGet = lambda x: [i for i, p in enumerate(self.timegap_list) if
                                             p[self.idproperty] == x[0] and p['team_id'] == x[1]]
    ...
    def initElimTeamTimeGap_list(self, matchrange_list):
        ...
        # gapindexerGet must have a (div_id, team_id) tuple passed to it
        self.timegap_indexerGet = lambda x: [i for i, p in enumerate(self.timegap_list) if
                                             p['div_id'] == x[0] and p['team_id'] == x[1]]
    ...
    def getElimcandidate_daytime(self, div_id, home, away, field_list,
        ...
        if team_list:
            teamgap_date_list = [self.timegap_list[self.timegap_indexerGet((div_id, team))[0]]['last_date'] for team in
                                 team_list]
            teamgap_endtime_list = [self.timegap_list[self.timegap_indexerGet((div_id, team))[0]]['last_endtime'] for
                                    team in team_list]
    ...
    def datesort_fields(self, mindate_tuple, field_list):
        ...
        datesortedfield_list = [{'date': k,
                                 'field_list': [{'field_id': x['field_id'], 'fieldday_id': x['fieldday_id']} for x in
                                                v]} for k, v in dategroup]
    ...
    def optimizeMatchOrder(self, rmlist):
        ...
        for divmatch_list in rmlist:
            ...
            for match in divmatch_list:
                ...
                cost = sum(10 * self.timegap_list[self.timegap_indexerGet((div_id, x))[0]]['fieldday_id'] for x in
                           (home, away))
                for x in (home, away):
                    ...
                    if last_endtime != -1:
                        ...
                        last_endtime_dt = datetime.combine(date.today(),
                                                           last_endtime)
                        cost += int(ceil(
                            (last_endtime_dt - parser.parse('08:00')).total_seconds() / _min_timegap.total_seconds()))
                match['cost'] = cost
    ...
    def reserveFieldTimeSlots(self, connected_div_list, field_list):
        ...
        total_gameday_slots = sum(
            len(self.tfstatus_list[self.tfindexerGet(f)]['slotstatus_list'][0]) for f in field_list)
