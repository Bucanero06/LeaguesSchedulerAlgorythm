...
from scheduler_service.util.schedule_util import all_value, enum, shift_list, \
...
from scheduler_service.util.sched_exceptions import FieldAvailabilityError, TimeSlotAvailabilityError, \
    FieldTimeAvailabilityError, \
    CodeLogicError, SchedulerConfigurationError, TimeCompactionError
...
class TimeBalancer(object):
    def __init__(self, fstatus_tuple):
        ...
        self._cel_indexerGet = lambda x: dict((p['div_id'], i) for i, p in enumerate(self._current_earlylate_list)).get(
            x)
        self._tel_indexerGet = lambda x: dict((p['div_id'], i) for i, p in enumerate(self._target_earlylate_list)).get(
            x)
    ...
    def get_divtarget_el(self, divinfo_tuple, fieldinfo_list, div_id,
        ...
        (home and away)'''
        print(f'{divinfo_tuple = }')
        print(f'{fieldinfo_list = }')
        print(f'{div_id = }')
    ...
    #     logging.debug("div fields bipartite graph %s %s effective edge sum for each node %s", df_biparG.nodes(),
    #                   df_biparG.edges(), sorted_edgesum_list)
    #
    ...
    #                       'ratio': len(connected_list) * float(divinfo_list[divinfo_indexerGet(x)]['totalteams']) / sum(
    #                           divinfo_list[divinfo_indexerGet(y)]['totalteams'] for y in connected_list)} for
    #                      connected_list in getConnectedDivisionGroup(fieldinfo_list) for x in connected_list]
    ...
    def getDivFieldEdgeWeight_list(self, divinfo_tuple, fieldinfo_list):
        """
        Calculates the 'fairness share' of fields for each soccer division based on a bipartite graph.

        This function is designed to ensure fair allocation of fields to different soccer divisions
        (e.g., age groups, skill levels) for scheduling games. It creates a bipartite graph, where one set of nodes
        represents divisions (e.g., Under-10, Under-12 soccer divisions) and the other set represents fields
        (e.g., soccer pitches). Edges in the graph connect divisions to their available fields.

        Parameters:
        - divinfo_tuple: Contains division information. Each division might have different characteristics like age group or skill level.
        - fieldinfo_list: List of available fields. Each field can be associated with one or more divisions.

        Returns:
        - A namedtuple (_List_Indexer) containing the product list of division IDs and their calculated proportional ratios,
          based on the field availability and division requirements, and an indexer function for easy access.

        The function follows these steps:
        1. Construct a bipartite graph representing the relationships between divisions and fields.
        2. For each connected group of divisions and fields, calculate the sum of 'fairness weights' for each division.
           The fairness weight represents how field allocation should be distributed among divisions.
        3. Combine these calculations with the ratio of team counts in each division to ensure proportional representation.
        4. Return a structured data type that allows easy retrieval of each division's 'fair share' of field allocation.
        """

        # Extract division information from the input tuple
        divinfo_list = divinfo_tuple.dict_list
        divinfo_indexerGet = divinfo_tuple.indexerGet
        ...
        df_biparG.add_nodes_from([x['div_id'] for x in divinfo_list], bipartite=0)
        # Connect divisions to fields based on availability or suitability
        df_biparG.add_edges_from([(x['div_id'], 'f' + str(y)) for x in divinfo_list for y in x['divfield_list']])
        ...
        # This is crucial when some fields are only available to certain divisions and not interconnected with others.
        for c in nx.connected_components(df_biparG):
            component = df_biparG.subgraph(c).copy()
            div_nodes, field_nodes = bipartite.sets(component)

            # Store nodes from each component
            all_div_nodes.extend(div_nodes)
            all_field_nodes.extend(field_nodes)

            # Calculate fairness share for each division based on field availability
            deg_fnodes = {f: component.degree(f) for f in field_nodes}
            edgesum_list = [{'div_id': d, 'edgesum': sum([1.0 / deg_fnodes[f] for f in component.neighbors(d)])} for d
                            in div_nodes]
            all_edgesum_lists.extend(edgesum_list)

        # Sort results for consistency
        sorted_edgesum_list = sorted(all_edgesum_lists, key=itemgetter('div_id'))

        # Adjust fairness share based on team counts in divisions
        divratio_list = [{'div_id': x,
                          'ratio': len(connected_list) * float(divinfo_list[divinfo_indexerGet(x)]['totalteams']) / sum(
                              divinfo_list[divinfo_indexerGet(y)]['totalteams'] for y in connected_list)} for
                         connected_list in getConnectedDivisionGroup(fieldinfo_list) for x in connected_list]
        ...
        # Final calculation: Multiply sorted edge sum list elements with sorted division ratio list elements
        prod_list = [{'div_id': x['div_id'], 'prodratio': x['edgesum'] * y['ratio']}
                     for (x, y) in zip(sorted_edgesum_list, sorted_divratio_list)]
        ...
        # Create an indexer function for easy access to the results
        prod_indexerGet = lambda x: dict((p['div_id'], i) for i, p in enumerate(prod_list)).get(x)
            ...
            if diff_cost > 0:
                ...
                for team_id in teams:
                        ...
                        for fieldday_id, slotstatus in enumerate(slotstatus_list, start=1):
                            ...
                            if any(isgame_list):
                                ...
                                if match_teams['div_id'] == div_id and \
                                    ...
                                    oppteam_id = match_teams[home_CONST] if match_teams[away_CONST] == team_id else \
                                    match_teams[away_CONST]
                                    oppteam_cost = self.getSingleTeamELstats(div_id, oppteam_id, el_type)
                                    logging.debug(
                                        "timebalancer:FindSwapMatchForTB: found slot0 field=%d div=%d fieldday=%d team=%d opp=%d",
                                    ...
                                    swapmatch_list = [{'swapteams': sstatus_list[x]['teams'],
                                                       'cost': self.getELstats(sstatus_list[x]['teams'],
                                                                               el_type).measure,
                                    ...
                                    if swapmatch_list:
                                        ...
                                        min_match_index = [i for i, j in enumerate(swapmatch_list) if
                                                           j['cost'] == min_cost]
                                        # create record that includes all the data
                                        # ********
                                        # including cost function - 'total_cost' below
                                        min_swapmatch_list.append(
                                            {'swapmatches': [x for i, x in enumerate(swapmatch_list)
                    ...
                    if min_swapmatch_list:
                        ...
                        ftstatus = \
                        self.fieldstatus_list[self.fstatus_indexerGet(field_id)]['slotstatus_list'][fieldday_id - 1][
                            'sstatus_list']
                        ...
                        if swap_teams is not swapmatches_sel['swapteams']:
                            raise CodeLogicError('ftscheduler:findSwapMatchTB:swap team mismatch %s' % (swap_teams,))
                        # do swap
...
'''
    ...
    def calcELdiff_list(self, div_id):
        ...
        el_diff_list = [
            {'team_id': i, 'early_diff': x[0]['early'] - x[1]['early'], 'late_diff': x[0]['late'] - x[1]['late']}
        ...
        while diff_sum > 0:
            ...
            for el_diff_list, div_id in zip(el_diff_list_list, div_set):
                ...
                status = self.FindSwapMatchForTimeBalance(div_id, fieldset,
                                                          diff_groups, el_type, random=random_count,
                                                          offset=offset_count)
            # recalculate EL differences after the swap
            el_diff_list_list = [self.calcELdiff_list(div_id) for div_id in div_set]
            prev_diff_sum = diff_sum
            diff_sum = sum(
                sum(x[diff_str] for x in el_diff_list if x[diff_str] > 0) for el_diff_list in el_diff_list_list)
            iteration_count += 1
    ...
    def ReTimeBalance(self, fieldset, connected_div_list):
        ...
        for el_type in ['early', 'late']:
            ...
            for i in range(3):
                ...
                if estatus:
                    logging.debug("timbalancer:retimebalance: eltype=%s divset=%s time balance SUCCEEDED", el_type,
                                  connected_div_list)
                    flag_dict[el_type] = True
                    break
            else:
                logging.debug("timbalancer:retimebalance: eltype=%s divset=%s ITERATION MAXED", el_type,
                              connected_div_list)
                    ...
                    if firstgame_ind != 0:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logging.error(
                                    "ftscheduler:compaction:field=%d gameday=%d no more gaps; gameday schedule is continuous and good",
                                    field_id, fieldday_id)
                                # all slots filled w. games, no False state
                                continue
                            else:
                                ...
                                logging.debug(
                                    "ftscheduler:compaction:blockshift for field=%d gameday=%d from ind=%d to %d",
    ...
    def shiftGameDaySlots(self, fieldstatus_round, isgame_list, field_id, fieldday_id, src_begin, dst_begin, shift_len):
        ...
        logging.debug(
            "ftscheduler:compaction:shiftGameDayslots isgamelist=%s, field=%d gameday=%d src_begin=%d dst_begin=%d len=%d",
