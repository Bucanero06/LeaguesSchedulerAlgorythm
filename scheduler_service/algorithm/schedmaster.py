...
class SchedMaster:
    ...
    def getHTMLTeamTable(self, div_age, div_gen, team_id):
        return_dict = self.get_schedule('team_id', team_id, div_age=div_age, div_gen=div_gen)
        game_list = return_dict['game_list']

        # Begin HTML table
        html = '<table style="width:100%; border:1px solid black;">'
        # Add caption
        html += f'<caption>{self.userid_name} {self.schedcol_name} {div_age}{div_gen}{team_id}</caption>'
        # Header row
        html += '<tr>'
        for header in ['Game Date', 'Start Time', 'Field', 'Home', 'Away']:
