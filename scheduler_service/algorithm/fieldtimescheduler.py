...
class FieldTimeScheduleGenerator:
    ...
    def generateSchedule(self, totalmatch_tuple, oddnumplay_mode=None, totalbyeteam_list=None):
        ...
        for connected_div_list in self.connected_div_components:
            ...
            if not self.fieldbalancer.validate_divteam_refcount(divrefdistrib_list,
                ...
                raise CodeLogicError(
                    "ftscheduler:generateSchedule: per-div and per-team reference count calc are not consistent per-div %s per-team  %s" % (
                    divrefdistrib_list, teamrefdistrib_list))
                return False
            for round_id in range(1, max(matchlist_len_list) + 1):
                ...
                rd_fieldcount_indexerGet = lambda x: dict(
                    (p['field_id'], i) for i, p in enumerate(rd_fieldcount_list)).get(x)
                ...
                for div_dict in submatch_list:
                    ...
                    matchlist_indexerGet = lambda x: dict((p['round_id'], i) for i, p in enumerate(divmatch_list)).get(
                        x)
                ...
                for rrgame in rrgenobj:
                    ...
                    hfindexerGet = lambda x: dict((p['field_id'], i) for i, p in enumerate(home_fieldmetrics_list)).get(
                        x)
                    afindexerGet = lambda x: dict((p['field_id'], i) for i, p in enumerate(away_fieldmetrics_list)).get(
                        x)
                    ...
                    if self.tminfo_indexerMatch and \
                        ...
                        for team_id in [home_id, away_id]:
                            ...
                            hf_list.append({'team_id': team_id,
                                            'af_list': self.tminfo_list[tmindex][
                                                'af_list'] if tmindex is not None else None})
                    else:
                        hf_list = None
                    ...
                    targetfieldcount_list = [{'field_id': x,
                                              'count': int(round(aggregnorm_list[agindexerGet(x)][
                                                                     'normweight'] * reqslots_perrnd_num))} for x in
                                             field_list]
                    ...
                    prefdays_prioritized_fields = self.day_balancer.assign_prefdays_priorities(div_id,
                                                                                               datesortedfield_list)
                    ...
                    if game_date != computedgame_date:
                        raise CodeLogicError(
                            "ftscheduler:generate: sstatus game_date %s does not match w computed game_date %s" % (
                            game_date, computedgame_date))
                    else:
                    ...
                    logging.info(
                        "div=%s%s home=%d away=%d round_id=%d, date=%s fieldday_id=%d field=%d gametime=%s slotindex=%d",
        ...
        for field_id in field_list:
            for fieldday_id, slotstatus_list in enumerate(
                    self.fieldstatus_list[self.fstatus_indexerGet(field_id)]['slotstatus_list'], start=1):
                            ...
                            for tid in ['home_id', 'away_id']:
                                ...
                                if team_id != byeteam_id:
                                    ...
                                    if cindex is not None:
                                        ...
                                        if self.check_later_date_time(current_cost, game_date, match['start_time']):
    ...
    def place_endschedule(self, field_list, match_list, round_game_date_list):
        ...
        for match in match_list:
            ...
            if nextstart is None:
                ...
                nextopen_ftstatus['teams'] = {'div_id': match['div_id'], home_CONST: match['home_id'],
                                              away_CONST: match['away_id']}
    ...
    def datesort_fields(self, minmaxdate_list, minmaxdate_indexerGet, field_list):
        ...
        before returning'''
        date_list = list(
            set([x['date'] for f in field_list for x in minmaxdate_list[minmaxdate_indexerGet(f)]['fielddate_list']]))
    ...
    def prioritizefield_list(self, datefield_list, sortedinput_list, key):
        ...
        prioritizednested_list = [
            [x for x in datefield_list if x['field_id'] in sortedinput_list[si_indexerGet(y)]['field_list']] for y in
            key_list]
    ...
    def findFieldGamedayLastTrueSlot(self, field_id, fieldday_id):
        sstatus_list = self.fieldstatus_list[self.fstatus_indexerGet(field_id)]['slotstatus_list'][fieldday_id - 1][
            'sstatus_list']
        isgame_list = [x['isgame'] for x in sstatus_list]
    ...
    def ProcessPreferences(self, fixteam_list):
        ...
        for cdiv_id in div_set:
            ...
            ginterval = divinfo['gameinterval']
            divfield_list = divinfo['divfield_list']
            gameinterval = timedelta(0, 0, 0, 0,
                                     ginterval)  # to be able to add time - see leaguedivprep fieldseasonstatus
            ...
            for constraint in divconstraint_list:
                ...
                for f in divfield_list:
                    ...
                    if cmap_index_list:
                        ...
                        if cstartafter_time and not cendbefore_time:
                            ...
                            if cendbefore_time > cstartafter_time:
                                ...
                                logging.debug(
                                    "ftscheduler:processprefs: constraint %d is not needed since start_after=end_before",
                                    cpref_id)
                        ...
                        if not firstgame_slot or not firstgame_slot['isgame']:
                            raise CodeLogicError(
                                "ftscheduler:ProccessContraints: firstgame for div %d, fieldday %d does not exist" % (
                                cdiv_id, cmapfieldday_id))
                        firstgame_time = firstgame_slot['start_time']  # first game time
                        startafter_index = self.mapStartTimeToSlot(cstartafter_time, firstgame_time,
                                                                   gameinterval) if cstartafter_time else None
                        if startafter_index and startafter_index > fslots_num - 1:
                            if segment_type == 3:
                                ...
                                raise SchedulerConfigurationError(
                                    "Constraint Configuration Error: Start after time is too late pref id %d div %d team %d" % (
                                    cpref_id, cdiv_id, cteam_id))

                        # -1 return means that the end time is before the end of the first game
                        endbefore_index = self.mapEndTimeToSlot(cendbefore_time, firstgame_time,
                                                                gameinterval) if cendbefore_time else -2
                        ...
                        if segment_type == 0:
                            ...
                            raise CodeLogicError(
                                "ftscheduler:process constraints - error with segment type, constraint %d" % (
                                cpref_id,))
                        ...
                        for slot_ind, slot_TF in enumerate(canswapTF_list):
                            if slot_TF:
                                ...
                                if fstatus_slot['isgame']:
                                    ...
                                    if swapdiv_id == cdiv_id and (home == cteam_id or away == cteam_id):
                                        logging.info(
                                            "ftscheduler:constraints: ***constraint satisfied with constraint=%d div=%d team=%d gameday=%d",
                                        ...
                                        if fixteam_list:
                                            ...
                                            if fixindex_list:
                                                ...
                                                divfixindexerGet = lambda x: dict(
                                                    (p['team_id'], i) for i, p in enumerate(divfixteam_list)).get(x)
                                        if 'swapped_priority' not in fstatus_slot \
                                                and (not divfixteam_list or (
                                                divfixindexerGet(home) is None and divfixindexerGet(away) is None)):
                            ...
                            logging.debug(
                                "ftscheduler:processprefs:candidate matches in field=%d constraint=%d for swap %s",
                    ...
                    status_int = self.findMatchSwapForConstraint(divfield_list,
                                                                 cdiv_id, cteam_id, cgame_date, cpriority,
                                                                 swapmatch_list,
                                                                 cdivteam_set)
    ...
    def findMatchSwapForConstraint(self, fset, div_id, team_id, game_date, priority, swap_list, divteam_set):
        ...
        if not fstatus_tuple:
            ...
            logging.debug("ftscheduler:findMatchSwapForConstraint:possible bye game for div %d team %d game_dat %s" % (
            div_id, team_id, game_date));
            return 0;
        ...
        fsstatus_list = \
        self.fieldstatus_list[self.fstatus_indexerGet(reffield_id)]['slotstatus_list'][reffieldday_id - 1][
            'sstatus_list']
        ...
        if samefieldindex_list:
            ...
            for swapindex in samefieldindex_list:
                ...
                if priority in PRIORITY_1_RANGE:
                    if refoppteam_id in divteam_set:
                        swap_cost = 2 * (
                                    swaphome_current_cost - swaphome_new_cost + swapaway_current_cost - swapaway_new_cost) + 2 * (
                                                refoppteam_new_cost + refteam_new_cost)
                    else:
                        swap_cost = 2 * (
                                    swaphome_current_cost - swaphome_new_cost + swapaway_current_cost - swapaway_new_cost) + (
                                                refoppteam_current_cost - refoppteam_new_cost) + 2 * refteam_new_cost
                elif priority in PRIORITY_2_RANGE:
                    if refEL_state and (swaphome_new_cost >= EL_cost_threshold or \
                                        swapaway_new_cost >= EL_cost_threshold or (
                                                swaphome_new_cost + swapaway_new_cost) >= 2 * EL_cost_threshold):
                        continue
            ...
            if cost_list:
                ...
                if max_swap['field_id'] != reffield_id:
                    raise CodeLogicError(
                        "ftschedule:findSwapMatchForConstraints reffield %d max_swap field do Not match" % (
                        reffield_id,))
                ...
                logging.debug(
                    "ftscheduler:swapmatchconstraints: swapping refslot %d with slot %d, refteams %s with teams %s on field %d fieldday %d",
                    refslot_index, max_swap_slot_index, refteams, max_swap_teams, reffield_id, reffieldday_id)
                print("****swapping refslot %d with slot %d, refteams %s with teams %s" % (
    ...
    def findFieldSeasonStatusSlot(self, fset, div_id, team_id, game_date):
        ...
        if break_flag:
            ...
            logging.debug("constraints: findswapmatch can't find slot for div=%d team=%d gameday=%d" % (
            div_id, team_id, fieldday_id))
            return None
    ...
    def getFieldSeasonStatus_list(self):
        ...
        for f in self.fieldinfo_list:
            ...
            calendarmap_indexerGet = lambda x: dict((p['fieldday_id'], i) for i, p in enumerate(calendarmap_list)).get(
                x)
        ...
        for connected_div in self.connected_div_components:
            ...
            for div_id in connected_div:
                ...
                if dayweek_set_len < div_numgdaysperweek:
                    ...
                    raise FieldTimeAvailabilityError(
                        "!!!!!!!!!!!!!!!! Not enough gamedays in week, need %d days, but only %d available" %
                ...
                if all_isless(totalfielddays_list, div_totalgamedays):
                    ...
                    raise FieldTimeAvailabilityError(
                        "!!!Not enough fielddays, need %d days, but only %d available for div %d" %
                ...
                if available_slots < required_slots:
                    ...
                    raise FieldTimeAvailabilityError(
                        "!!!Not enough total field and time slots, need %d slots, but only %d available" %
    ...
    def init_homefieldweight_list(self):
        ...
        for div_id in div_id_list:
            ...
            if index_list:
                ...
                for index in index_list:
                    ...
                    effweight_list = [
                        {'field_id': x, 'effweight': weight} if x in af_list else {'field_id': x, 'effweight': 0.0} for
                        x in divfield_list]
        ...
        if not self.tminfo_indexerGet:
            self.tminfo_indexerGet = lambda x: dict((p['dt_id'], i) for i, p in enumerate(self.tminfo_list)).get(
    ...
    def calc_divreffield_distribution_list(self, ngperteam_list):
        ...
        for ngperteam_dict in ngperteam_list:
            ...
            distribution_list = [
                {'field_id': x['field_id'], 'sumcount': x['aggregweight'] * inv_sumweight * div_totalgames} for x in
                hfweight_list]
