...
def get_team_metrics(divteam_metrics, div_id, team_id):
    return next((t for d in divteam_metrics for t in d['team_data']
                 if d['div_id'] == div_id and t['team_id'] == team_id), None)
...
def calculate_team_costs(candidate, team_config, pref_unmet_sched, team_metrics,
    ...
    opponent_cost = calculate_opponent_team_cost(candidate, team_config, pref_unmet_sched,
                                                 divteam_metrics, prefdays_list, candidate_swap_day,
                                                 pref_unmet_swap_day)
...
def find_swap_candidates(team_config, pref_unmet_sched, divteam_metrics):
    candidates = [{'div_id': x['div_id'], 'swap_sched': z} for x in divteam_metrics for y in x['team_data']
                  ...
                  and z['round_id'] == pref_unmet_sched['round_id'] and z['game_date'].weekday() in team_config[
                      'prefdays']]
    return candidates
...
def calc_fairness(counters, fair_games_per_day):
    """
    Calculate fairness of games distribution.

    The function computes the fairness of the distribution of games among
    different counters. It returns a float where 1.0 indicates perfect
    fairness (each counter's count is equal to fair_games_per_day), and
    values closer to 0.0 indicate less fairness.

    Parameters:
    counters (list of dict): A list of dictionaries where each dictionary
                             has a 'count' key with an integer value.
    fair_games_per_day (int): Target number of fair games per day.

    Returns:
    float: Fairness measure of the distribution of games.

    Raises:
    ZeroDivisionError: If fair_games_per_day is zero.

    Example:
    >>> calc_fairness([{'count': 5}, {'count': 7}, {'count': 6}], 6)
    0.944911182523068

    Note:
    Ensure fair_games_per_day is not zero to avoid division by zero errors.
    """
